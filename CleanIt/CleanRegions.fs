...
type MemberCollector() = 
   ...
   let (|DP|_|) (members: Map<MemberType, Map<string, MemberDeclarationSyntax>>) (signame: string) = 
      let name = if signame.StartsWith("prop:") then "field:" + signame.Substring(5) else "field:" + signame
      ...
      if members.[Field].ContainsKey fieldName = false then None else
         ...
         if fp.Declaration.Type.ToString() = "DependencyProperty" && (isStatic fp.Modifiers) then Some(DependencyProperty(name, fp, null)) else None
